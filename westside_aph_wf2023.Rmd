...
#Summed DPR
SummedDPR_Product<- ddply(dpr_data2023, c("TRT","Product","Block",  "DPR_Label", "DAT"), summarise,
                    WF.Nym.Sum = sum(WF.Nym, na.rm=TRUE),
                    WF.Ad.Sum = sum(WF.Ad, na.rm=TRUE),
                    Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))
...
SummedDPR.Avg.proudct.1 <- SummedDPR.Avg.proudct.1[order(SummedDPR.Avg.proudct.1$DPR_Label),]
...
WF.Nym_total<-lm(sqrt(WF.Nym.Sum)~DPR_Label, data = SummedDPR_Product)


Anova(WF.Nym_total)

posthoc_results <- lsmeans(WF.Nym_total, "DPR_Label") 
  
cld_letters <- cld(posthoc_results, Letters = letters)
...
XX<-as.data.frame(cld_letters[order(cld_letters),])
...
aphid_total<-lm(sqrt(Aph.Tot.Sum)~DPR_Label,data = SummedDPR_Product)
Anova(aphid_total)

LS_aphid_total <- emmeans(aphid_total, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_aphid_total$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_aphid_total;CLD_aphid_total
...
XX<-as.data.frame(CLD_aphid_total[order(CLD_aphid_total),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
...
WFAdultChart_dpr2023_app1 + geom_text(data=XX ,aes(x=DPR_Label,y=WF.Ad.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
...
TotalAphidChart_dpr2023_top_app2 <- SummedDPR.App2.Avg %>%
  ...
  labs(fill="DAT") + 
  ggtitle("App. 2: Top leaf")+
  coord_cartesian(ylim=c(0,(max(XX$Aph.Tot.SumAvg*2.5))))+ 
  geom_text(data=XX ,aes(x=DPR_Label,y=Aph.Tot.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5)
...
```{r}
SummedDPR.App2.Avg %>%
  dplyr::filter (Leaf_Location == "Top", DPR_Label == "Buprofezin_StdCov")%>%
  dplyr::group_by(DAT)%>%
  dplyr::summarise(sum= sum(Aph.Tot.SumAvg))
```
