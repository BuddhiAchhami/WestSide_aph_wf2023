...
WFNymChart_dpr2023_cld_top_app1 <- SummedDPR.App1.Avg %>%
  ...
  geom_text(data=XX ,aes(x=DPR_Label,y=WF.Nym.SumAvg,label=group,fill=NULL),
...
SummedWF_bottom_app1 <-  SummedWF %>%
   filter (Leaf_Location == "Bottom", App == "App1")

WF.Nym_Summed_bottom_app1<-lm(sqrt( WfNym.Sum)~DPR_Label+ Block,data = SummedWF_bottom_app1)
#WF.Nym_Summed <- glm.nb(round(WF.Nym.Sum,0)~DPR_Label+Block,data=SummedDPR)
...
SummedDPR.Avg_bottom1

# Assigning CLD letters
XX<-as.data.frame(CLD_WF.Nym_Summed_bottom_app1[order(CLD_WF.Nym_Summed_bottom_app1$DPR_Label),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$WF.Nym.SumAvg<-SummedDPR.Avg_bottom1$WF.Nym.SumAvg


WFNymChart_dpr2023_cld_bottom_app1 <- SummedDPR.App1.Avg %>%
  ...
  labs(fill = "DAT")+
  ylim(0,5)+
  geom_text(data=XX ,aes(x=DPR_Label,y=WF.Nym.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5)+
...
WFNymChart_dpr2023_cld_bottom_app1
```
