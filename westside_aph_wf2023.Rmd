...
WFNymChart_dpr2023_top_app1 <- SummedDPR.App1.Avg %>%
  ...
  ylim(0,2) +
  geom_text(data=XX[complete.cases(XX),], aes(x=DPR_Label, y=WF.Nym.SumAvg, label=group), vjust=-1, hjust=.5)
...
WFAdultChart_dpr2023_app1 <- SummedDPR.App1.Avg %>%
  ...
  ggtitle("App. 1") +
  ylim(0,2)+
  geom_text(data=XX ,aes(x=DPR_Label,y=WF.Ad.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5)
...
ggsave(file="TotalAphidChart_dpr2023_bottom_app2.jpg", width=10, height=6, dpi=600)
```
