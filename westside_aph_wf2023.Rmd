...
dpr_data2023.App2 <- dpr_data2023 %>%
  filter (DAT %in% c("3-DAT2","7-DAT2","10-DAT2", "13-DAT2","21-DAT2"))
...
#Summed DPR
SummedDPR<- ddply(dpr_data2023, c("TRT","Product","DPR_Label","Block", "Leaf_Location"), summarise,
                    WF.Nym.Sum = sum(WF.Nym, na.rm=TRUE),
                    WF.Ad.Sum = sum(WF.Ad, na.rm=TRUE),
                    Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))

SummedDPR.Avg<- ddply(SummedDPR, c("TRT","Product","DPR_Label", "Leaf_Location"), summarise,
...
SummedDPR.App2<- ddply(dpr_data2023.App2, c("TRT","Product","DPR_Label","Block", "Leaf_Location" ), summarise,
                    WF.Nym.Sum = sum(WF.Nym, na.rm=TRUE),
                    WF.Ad.Sum = sum(WF.Ad, na.rm=TRUE),
                    Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))

SummedDPR.App2.Avg<- ddply(SummedDPR.App2, c("TRT","Product","DPR_Label", "Leaf_Location"), summarise,
                    N = sum(!is.na(WF.Nym.Sum)),
                    WF.Nym.SumAvg = mean(WF.Nym.Sum, na.rm=TRUE),
                    WF.Nym.SumSE   = sd(WF.Nym.Sum, na.rm=TRUE) / sqrt(N),
                    WF.Ad.SumAvg = mean(WF.Ad.Sum, na.rm=TRUE),
...
Summed_all<- ddply(dpr_data2023_selected, c("Product","TRT","Block", "Leaf_Location", "DPR_Label", "App"), summarise,
                 WfNym.Sum = sum(WF.Nym, na.rm=TRUE),
                 WfAd.Sum = sum(WF.Ad, na.rm=TRUE),
                 Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))
...
```{r}
# Combined model: whitefly nymphs, whitefly adults, aphid total
...
WF.Nym_Summed<-lm(sqrt(WfNym.Sum)~ DPR_Label+Block+  Leaf_Location + App, data = Summed_all)
Anova(WF.Nym_Summed)


WFAd_Summed<-lm(sqrt(WfAd.Sum)~ DPR_Label + Block+ Leaf_Location + App, data = Summed_all)
Anova(WFAd_Summed)


Aph.Tot_Summed<-lm(sqrt(Aph.Tot.Sum)~ DPR_Label +Block+ Leaf_Location + App, data = Summed_all)
Anova(Aph.Tot_Summed)
```
...
# Application 1
Summed_app1 <- Summed_all %>%
  filter (App == "App1")

# Whitefly nymphs
WF.Nym_Summed_app1<-lm(sqrt(WfNym.Sum)~ DPR_Label + Leaf_Location, data = Summed_app1)
Anova(WF.Nym_Summed_app1)

# Whitefly adults
WF.Adult_Summed_app1<-lm(sqrt(WfAd.Sum)~ DPR_Label +  Leaf_Location, data = Summed_app1)
Anova(WF.Adult_Summed_app1)

#Aphid adult
Aphid_Summed_app1<-lm(sqrt(Aph.Tot.Sum)~ DPR_Label +  Leaf_Location, data = Summed_app1)
...
Summed_app2 <- Summed_all %>%
  filter (App == "App2")
# Whitefly nymphs
WF.Nym_Summed_app2<-lm(sqrt(WfNym.Sum)~ DPR_Label+Block +  Leaf_Location, data = Summed_app2)
Anova(WF.Nym_Summed_app2)

# Whitefly adults
WF.Adult_Summed_app2<-lm(sqrt(WfAd.Sum)~ DPR_Label+Block +  Leaf_Location, data = Summed_app2)
Anova(WF.Adult_Summed_app2)

#Aphid adult
Aphid_Summed_app2<-lm(sqrt(Aph.Tot.Sum)~ DPR_Label+Block +  Leaf_Location, data = Summed_app2)
...
# Application 1
 ...
 SummedWF_top_app1 <-  SummedWF %>%
   filter (Leaf_Location == "Top", App == "App1")

 SummedWF_top_app1

WF.Nym_Summed_top_app1<-lm(sqrt(WfNym.Sum)~DPR_Label,data = SummedWF_top_app1)
...
WF.Nym_Summed_top_app1 <- emmeans(WF.Nym_Summed_top_app1, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(WF.Nym_Summed_top_app1$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_WF.Nym_Summed_top_app1;CLD_WF.Nym_Summed_top_app1
```
##plotting

```{r}
# Assigning CLD letters
XX<-as.data.frame(CLD_WF.Nym_Summed_top_app1[order(CLD_WF.Nym_Summed_top_app1$DPR_Label),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$WF.Ad.SumAvg<-SummedDPR.Avg$WF.Ad.SumAvg



WFNymChart_dpr2023_cld_top_app1 <- SummedWF %>%
  filter (Leaf_Location == "Top")%>%
  filter (App == "App1")%>%
  ggplot(aes(fill=DAT, y=WfNym.Sum, x=DPR_Label)) +
  ...
  geom_text(label=cld_letters, aes(x=DPR_Label, y=WfNym.Sum , label=group, fill=NULL),
            position=position_stack(vjust=0.5), size=3) +
  theme(legend.position="none")



WFNymChart_dpr2023_cld_top_app1
```
